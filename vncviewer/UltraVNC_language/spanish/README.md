# Spanish Language Resources for UltraVNC

This folder contains the Spanish language resource files for the UltraVNC Viewer application.

## Files Included

### Resource Files (Ready for Translation)
- `vncviewer.rc` - Main Spanish resource script with paths to original resources
...
1. **Edit Dialog Templates**: Open `vncviewer.rc` and translate all dialog text, menus, and string table entries to Spanish
...
## Building the Spanish DLL

To build the Spanish language DLL:
1. Update the project file to reference `spanish\vncviewer.rc`
2. Build the UltraVNC_language project
3. Output: `UltraVNC_language.dll` (containing Spanish text with original visual resources)

## Usage

When built, the resulting DLL can be loaded by UltraVNC Viewer to provide Spanish interface:
```cpp
HMODULE hSpanishDLL = LoadLibrary(L"UltraVNC_language.dll");
```
...
- Ready for Spanish translation of all UI elements while maintaining original visual assets
