    ...
    for (y = ry; y < ry+rh; y += 16) {											\
		for (x = rx; x < rx+rw; x += 16) {										\
            ...
            if (subencoding & rfbHextileRaw) {									\
                ...
                if (m_DIBbits) ConvertAll(w,h,x, y,bpp/8,(BYTE *)m_netbuf,(BYTE *)m_DIBbits,m_si.framebufferWidth);\
                continue;														\
            }																	\
																				\
            if (subencoding & rfbHextileZlibRaw) {								\
		        ...
		        if (zlibDecompress((unsigned char *)m_netbuf, m_zlibbuf, nCompData, ((w*h+2)*(bpp/8)), &m_decompStreamRaw)) {  \
                    if (m_DIBbits) ConvertAll(w,h,x, y,bpp/8,(BYTE *)m_zlibbuf,(BYTE *)m_DIBbits,m_si.framebufferWidth);\
				}																\
                continue;														\
            }																	\
																				\
            if (subencoding & rfbHextileZlibHex) {								\
		        ...
		        if (zlibDecompress((unsigned char *)m_netbuf, m_zlibbuf, nCompData, ((w*h+2)*(bpp/8)+20), &m_decompStreamEncoded)) {  \
                    HandleZlibHexSubencodingBuf##bpp(x, y, w, h, subencoding, m_zlibbuf,&bgcolor,&fgcolor);							\
				}																\
                continue;														\
            }																	\
			else {																\
				HandleZlibHexSubencodingStream##bpp(x, y, w, h, subencoding,&bgcolor,&fgcolor);	\
			}																	\
																				\
        }																		\
    }																			\
																				...
																				\
void ClientConnection::HandleZlibHexSubencodingStream##bpp(int x, int y, int w, int h, int subencoding,CARD##bpp *bgcolor,CARD##bpp *fgcolor)		\
{																				\
    CARD##bpp bg, fg;															\
    int i;																		\
    CARD8 *ptr;																	\
	...
	FillSolidRect##bpp(x,y,w,h,bgcolor);												\
																				\
	...
	if (subencoding & rfbHextileSubrectsColoured) {								\
		...
		for (i = 0; i < nSubrects; i++) {										\
			*fgcolor =*(CARD##bpp *)ptr;						\
			ptr += (bpp/8);														\
			sx = *ptr >> 4;														\
			sy = *ptr++ & 0x0f;													\
			sw = (*ptr >> 4) + 1;												\
			sh = (*ptr++ & 0x0f) + 1;											\
			FillSolidRect##bpp(x+sx, y+sy, sw, sh, fgcolor);							\
		}																		\
																				\
	} else {																	\
		...
		for (i = 0; i < nSubrects; i++) {										\
			...
			FillSolidRect##bpp(x+sx, y+sy, sw, sh, fgcolor);							\
																				...
																				\
																				\
void ClientConnection::HandleZlibHexSubencodingBuf##bpp(int x, int y, int w, int h, int subencoding, unsigned char *buffer,CARD##bpp *bgcolor,CARD##bpp *fgcolor)		\
{																				\
	CARD##bpp bg, fg;															\
	int i;																		\
	CARD8 *ptr;																	\
