...
bool VNCviewerApp32::NewConnection() {
...
bool VNCviewerApp32::NewConnection(bool val1, bool val2,bool val3) {
		ClientConnection *pcc = new ClientConnection(this);
		try {
			pcc->Global_options();
			}
...
void VNCviewerApp32::StartListener() {
	if (m_pdaemon == NULL && m_pflasher == NULL)
	{
	try {
			// m_pflasher = new Flasher(FLASH_PORT_OFFSET);
			m_pflasher = new Flasher(m_options.m_listenPort + FLASH_PORT_OFFSET - INCOMING_PORT_OFFSET);
...
void VNCviewerApp32::StopListener() {
	if (m_pdaemon != NULL && m_pflasher != NULL)
	{
	try {
			delete m_pflasher;
...
void VNCviewerApp32::NewConnection(TCHAR *file) {
	ClientConnection *pcc = new ClientConnection(this, file);
	try {
		pcc->Run();
	} catch (Exception &e) {
		e.Report();	
