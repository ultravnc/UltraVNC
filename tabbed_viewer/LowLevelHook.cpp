...
void LowLevelHook::Enable_Disable(bool state)
{
        g_fHookActive=state;
}
...
BOOL LowLevelHook::HaveFocus() 
{
// Check if mouse is over inputwindow
	POINT point;
	GetCursorPos(&point);
	HWND testhwnd=WindowFromPoint(point);
    if (testhwnd==g_hwndVNCViewer) 
	{
		if (GetFocus()==testhwnd)
		{
			return true;
		}
	}
