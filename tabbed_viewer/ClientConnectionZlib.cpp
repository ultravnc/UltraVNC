...
void ClientConnection::ReadZlibRect(rfbFramebufferUpdateRectHeader *pfburh,int XOR) {
	...
	else if (XOR==0)
	{
		mybool *maskbuffer=(mybool *)m_zlibbuf;
		BYTE *databuffer=m_zlibbuf+(((pfburh->r.w*pfburh->r.h)+7)/8);
		ConvertAll(pfburh->r.w,pfburh->r.h,pfburh->r.x, pfburh->r.y,m_myFormat.bitsPerPixel/8,(BYTE *)m_zlibbuf,(BYTE *)m_DIBbits,m_si.framebufferWidth);
	}
}
...
void ClientConnection::ReadSolidRect(rfbFramebufferUpdateRectHeader *pfburh) {
	...
	SETUP_COLOR_SHORTCUTS;
		...
		switch (m_myFormat.bitsPerPixel) {
		case 8:
			FillSolidRect8(pfburh->r.x, pfburh->r.y, pfburh->r.w, pfburh->r.h, (CARD8*)m_netbuf);
				break;
		case 16:
			FillSolidRect16(pfburh->r.x, pfburh->r.y, pfburh->r.w, pfburh->r.h, (CARD16*)m_netbuf);
				break;
		case 24:
		case 32:
			FillSolidRect32(pfburh->r.x, pfburh->r.y, pfburh->r.w, pfburh->r.h, (CARD32*)m_netbuf);           
				break;
		default:
			vnclog.Print(0, _T("Invalid number of bits per pixel: %d\n"), m_myFormat.bitsPerPixel);
			return;
		...
		if (surh.encoding==rfbEncodingSolidColor)
			{
				...
				switch (m_myFormat.bitsPerPixel) {
				case 8:
					FillSolidRect8(surh.r.x, surh.r.y, surh.r.w, surh.r.h, (CARD8*)pzipbuf);
					//SETSOLPIXELS(pzipbuf,8, surh.r.x, surh.r.y, surh.r.w, surh.r.h)
					break;
				case 16:
					FillSolidRect16(surh.r.x, surh.r.y, surh.r.w, surh.r.h, (CARD16*)pzipbuf);
					//SETSOLPIXELS(pzipbuf,16, surh.r.x, surh.r.y, surh.r.w, surh.r.h)
					break;
				case 24:
				case 32:
					FillSolidRect32(surh.r.x, surh.r.y, surh.r.w, surh.r.h, (CARD32*)pzipbuf);
			...
			{
				UINT numpixels = surh.r.w * surh.r.h;
				ConvertAll( surh.r.w, surh.r.h, surh.r.x,  surh.r.y,m_myFormat.bitsPerPixel/8,(BYTE *)pzipbuf,(BYTE *)m_DIBbits,m_si.framebufferWidth);
