...
,"  -I   don't scan through Image files"
  ...
  {
    printff(text[i], VERSION, REVDATE);
    putchar('\n');
  }
#ifdef DOS
  check_for_windows("Zip");
...
# endif
  ...
  {
    printff(versinfolines[i], "Zip", VERSION, REVDATE);
    putchar('\n');
  }
  ...
  if (strcmp(ZLIB_VERSION, zlibVersion()) == 0)
    printff("\tUSE_ZLIB [zlib version %s]\n", ZLIB_VERSION);
  else
    printff("\tUSE_ZLIB [compiled with version %s, using version %s]\n",
      ZLIB_VERSION, zlibVersion());
  i++;  /* zlib use means there IS at least one compilation option */
#endif
#if CRYPT
  printff("\t[encryption, version %d.%d%s of %s]\n",
      ...
      if (argv[i][1])
        for (p = argv[i]+1; *p; p++)
          switch (*p)
          {
            ...
            case 'a':
              aflag = ASCII;
              printff("Translating to ASCII...\n");
              break;
#endif /* EBCDIC */
#ifdef CMS_MVS
            case 'B':
              bflag = 1;
              printff("Using binary mode...\n");
              break;
#endif /* CMS_MVS */
#ifdef TANDEM
            case 'B':
