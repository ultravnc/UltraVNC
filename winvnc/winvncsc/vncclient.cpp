...
//	m_client->m_fullscreen = m_client->m_encodemgr.GetSize();
	...
	if (!m_server->GetName(desktopname, &desktopnamelen)) {
#endif
...
vncClient::SendUpdate(rfb::SimpleUpdateTracker &update)
{
	...
	if (update.is_empty() && !m_cursor_update_pending && !m_NewSWUpdateWaiting) return FALSE;
...
void vncClient::UndoFTUserImpersonation()
{
	...
	if (lTime - m_lLastFTUserImpersonationTime < 30000) return;
