# Spanish Language Resources for UltraVNC Server

This folder contains the Spanish language resource files for the UltraVNC Server application.

## Files Included

### Resource Files (Ready for Translation)
- `winvnc.rc` - Main Spanish resource script with dialogs, menus, and string tables
- `UltraVNC_Server_language_Spanish.vcxproj` - Visual Studio project file for building the Spanish DLL
...
Build the project to create the Spanish language DLL:
- Project file: `UltraVNC_Server_language_Spanish.vcxproj`
- Output DLL: `winvnclang_Spanish.dll`
- Resource reference: `winvnc.rc`

## Usage

When built, the resulting DLL can be loaded by UltraVNC Server to provide Spanish interface:
```cpp
HMODULE hSpanishDLL = LoadLibrary("winvnclang_Spanish.dll");
Load_Localization(hSpanishDLL);
```
